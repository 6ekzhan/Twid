...
class PlacesInfo {
  static Future<List<String>> getPlacesId(LatLng coordinates) async {
    ...
    final String url =
        "https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${coordinates.latitude},${coordinates.longitude}&key=$google_api_key&radius=800";
  ...
  static Future<List<dynamic>> getPlaces(List<String> placesId) async {
    List res = [];
    for (var i = 0; i < ((placesId.length > 3) ? 3 : placesId.length); i++) {
      final String uri =
